...
// 商品id
func productIDs(c []*pb.CartItem) []string {
	res := make([]string, len(c))
	for i, v := range c {
		res[i] = v.GetProductId()
	}
...
func (fe *FrontendServer) ProductHandler(ctx *gin.Context) {
	...
	id := ctx.Param("id")
	if id == "" {
		renderHTTPError(log, ctx, errors.New("商品id没有指定"), http.StatusInternalServerError)
		return
	}

	log.WithField("id", id).WithField("currency", currentCurrency(r)).Debug("商品服务")

	p, err := fe.getProduct(r.Context(), id)
	if err != nil {
		renderHTTPError(log, ctx, errors.Wrap(err, "不能查询到商品：%s"), http.StatusInternalServerError)
		return
	}

	currencies, err := fe.getCurrencies(r.Context())
	if err != nil {
		renderHTTPError(log, ctx, errors.Wrap(err, "不能查询到货币：%s"), http.StatusInternalServerError)
		return
	}
	...
	price, err := fe.convertCurrency(r.Context(), p.GetPriceUsd(), currentCurrency(r))
	if err != nil {
		renderHTTPError(log, ctx, errors.Wrap(err, "货币转换失败：%s"), http.StatusInternalServerError)
		return
	}
...
func (fe *FrontendServer) addToCartHandler(ctx *gin.Context) {
	...
	if productID == "" || quantity == 0 {
		renderHTTPError(log, ctx, errors.New("无效表单输入"), http.StatusInternalServerError)
		return
	}
...
func (fe *FrontendServer) viewCartHandler(ctx *gin.Context) {
	...
	for i, item := range cart {
		...
		if err != nil {
			renderHTTPError(log, ctx, errors.Wrapf(err, "查询不到商品：%v", item.GetProductId()), http.StatusInternalServerError)
			return
		}

		price, err := fe.convertCurrency(r.Context(), p.GetPriceUsd(), currentCurrency(r))
		if err != nil {
			renderHTTPError(log, ctx, errors.Wrapf(err, "不能转换货币：%v", err), http.StatusInternalServerError)
			return
		}
...
func (fe *FrontendServer) setCurrencyHandler(ctx *gin.Context) {
	...
	log.WithField("curr.new", cur).WithField("curr.old", currentCurrency(r)).Debug("setting currency")
	if cur != "" {
